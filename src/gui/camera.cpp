...
QVector3D Camera::getEye() const {
    ...
    const float sineAzimuth = sinf(m_azimuthAngle);
    const float cosineAzimuth = cosf(m_azimuthAngle);
    const float sinePolar = sinf(m_polarAngle);
    const float cosinePolar = cosf(m_polarAngle);
