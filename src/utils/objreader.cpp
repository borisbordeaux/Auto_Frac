...
void poly::reader::readOBJ(QString const& filename, poly::Mesh& mesh) {
    QFile file(filename);
    ...
    if (file.open(QIODevice::ReadOnly | QIODevice::Text)) {
        ...
        while (!stream.atEnd()) {
            ...
            if (list.size() == 4 && !list[0].compare("v")) {
                //create the vertex
                poly::Vertex* v = new poly::Vertex(list[1].toFloat(), list[2].toFloat(), list[3].toFloat(), QString::number(mesh.vertices().size() + 1));
